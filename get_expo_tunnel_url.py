#!/usr/bin/env python3
"""
Get the actual Expo tunnel URL from port 8082
"""
...
def get_expo_tunnel_url():
    ...
    try:
        # Check if Expo is running on port 8082
        response = requests.get('http://localhost:8082', timeout=5)
        if response.status_code == 200:
            print("✅ Expo server is running on port 8082")
            ...
            tunnel_url = f"exp://{local_ip}:8082"
            return tunnel_url
...
def main():
    print("🚀 Getting Expo Tunnel URL (Port 8082)")
    print("=" * 50)
    ...
    # Create QR code
    print("\n📱 Creating QR code...")
    if create_qr_code(url, "expo_tunnel_8082_qr.png"):
        print(f"✅ QR code saved: expo_tunnel_8082_qr.png")
        print(f"🔗 URL: {url}")
        print("\n🎯 Next steps:")
